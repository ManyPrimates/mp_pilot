...
First an overview of the data, plotted by species and condition. Small, transparent dots represent aggregated data for each individual by condition. Open dots are the group mean for that condition. Error bars are 95% confidence intervals. Text labels are the sample size.

```{r overview by species, warning=FALSE}
plot_individual <- mp_data %>% 
  group_by(phylo, clade, species, condition, subject_site) %>%
...
p2 + aes(x = condition, y = correct, col = condition) +
...
# Overview by species across conditions

```{r overview by species across conditions, warning=FALSE}
...
```{r}
ggsave('../graphs/02_02_overview_across_conditions.png', width = 4, height = 1.8, scale = 2)
```
...
```{r, fig.width=4, fig.height=1.2}
p_taskexp_chimp = p2 + aes(x = condition, y = correct) +
  geom_jitter(data = chimp_task_plot_individual, aes(col = site), width = .3, height = .015, alpha = .5, size = 3) +
  geom_pointrange(data = chimp_task_plot_group, aes(y = mean, ymin = ci_lower, ymax = ci_upper, shape = condition), size = .8, stroke = 1.5) +
  ...
  scale_shape_manual('delay', values = c(1, 2, 5), breaks = c('short', 'medium', 'long')) +
  scale_colour_solarized() +
  ggtitle("Chimpanzees") +
  coord_flip()
```
...
```{r, fig.width=4, fig.height=1.2}
p_taskexp_bon = p2 + aes(x = condition, y = correct) +
  geom_jitter(data = bonobo_task_plot_individual, aes(col = site), width = .3, height = .015, alpha = .5, size = 3) +
  geom_pointrange(data = bonobo_task_plot_group, aes(y = mean, ymin = ci_lower, ymax = ci_upper, shape = condition), size = .8, stroke = 1.5) +
...
plot_age <- mp_data %>%
  ...
  group_by(subject_site, norm_age, clade, species, condition) %>%
  summarise(correct = mean(correct))
```
...
ggplot(plot_age, aes(x = norm_age, y = correct))+
  ...
  geom_smooth(aes(col = condition), method = "lm", formula = y ~ poly(x, 3), show.legend = F) +
...
```{r}
ggsave('../graphs/02_09_age_by_condition.png', width = 4, height = 1.3, scale = 2)
```
...
ggplot(plot_age, aes(x = norm_age, y = correct))+
  ...
  geom_vline(xintercept = 0, lty = 2) +
  geom_hline(yintercept = 1/3, lty = 2) +
  facet_grid(condition ~ clade, scales = 'free_x') +
  labs(x = "Normed Age (relative to species longevity)", y = "Proportion Correct") +
...
```{r}
ggsave('../graphs/02_10_age_by_condition_species.png', width = 4, height = 3.4, scale = 2)
```
...
```{r}
plot_cup <- mp_data %>%
   group_by(subject_site, cup_distance, clade, species, condition) %>%
   summarise(correct = mean(correct))
```
