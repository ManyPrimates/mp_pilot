...
chimp_task_plot_group <- chimp_task_plot_individual %>%
   multi_boot_standard(col = "correct")

ggplot() +
  geom_jitter(data = chimp_task_plot_individual, aes(x = condition, y = correct, col = condition),width = .3,height = .015, alpha = .5)+
  geom_pointrange(data = chimp_task_plot_group, aes(x = condition, y = mean, col = condition,ymin = ci_lower, ymax = ci_upper),size = .8)+
  geom_hline(yintercept = 1/3, lty=2)+
  labs(x="",y="proportion correct")+
...
plot_age <- mp_data %>%
   group_by(subject_site,board_size,species,condition) %>%
   summarise(correct = mean(correct))


ggplot(plot_age, 
       aes(x = board_size, y = correct))+
  geom_point(aes(colour=species))+
  geom_smooth(method = "lm", se = F , level = 0.95)+
  guides(fill=FALSE)+
  geom_hline(yintercept = 1/3, lty=2)+
