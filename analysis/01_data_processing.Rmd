...
Note. Sweetwaters is represented twice (for groups 1 and 2)
...
> Any cases where the `correct` value doesn't match whether `hiding_location` and `pick` are identical?

```{r}
raw_data %>%
  filter(as.numeric(hiding_location == pick) != correct) %>%
  select(site:subject_site, trial:researcher)
```
