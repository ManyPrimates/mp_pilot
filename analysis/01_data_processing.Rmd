...
kris <- kris_raw %>%
  ...
  mutate(test_situation = ifelse(test_situation == "solitary", "single", "group"),
         species = recode(species, 'black faced spider monkeys' = 'black faced spider monkey'),
...
kuma <- kuma_raw %>%
  select(-`Second Coder`) %>%
  mutate(condition = recode(condition, "0s" = "short", "15s" = "medium", "30s" = "long"))
```
...
```{r}
doChecks(dpz_lem_raw)
```

> **!!!** Not sure what's up with this line in which `pick = 9`. If this is supposed to be NA, then exclude (not currently the case). If it's a different pick (1, 2, 3), correct this. --jw

```{r}
filter(dpz_lem_raw, pick == 9)
```
...
> **!!!** Where does the new board_size number (61.5) come from? --jw

## Language Research Center

```{r}
# specify column types -- makes 1st col (date) text, guesses (as usual) for everything else
ct = c('text', rep('guess', nrow(sample) - 1))
```
...
mkyhv_mcqs <- mkyhv_mcqs_raw %>%
  mutate(pick = as.numeric(pick),
         correct = as.numeric(correct),
         species = gsub(" ", "_", species),
         test_situation = recode(test_situation, "double" = "group"),
         secondary_pick = as.character(secondary_pick)) %>%
  filter(!is.na(pick))
```
...
```{r write data file}
write_csv(raw_data, "../data/merged_data/01_manyprimates_pilot_merged_data_20181007.csv")
```
